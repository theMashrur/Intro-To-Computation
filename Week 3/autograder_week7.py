...
def question1_f(x1, x2, x3):
    ...
    number_of_tests = 3
    try:
        npt.assert_approx_equal(x1, 3.09162380666784)
    except:
    ...
    try:
        npt.assert_approx_equal(x2, 3.1415919276751456)
    except:
    ...
    try:
        npt.assert_approx_equal(x3, 3.1415926535714034)
    except:
...
def question2e_plot(_globals):
    ...
    t = [t1, t2, t3]
    x = np.asarray([np.arange(1, 21), np.arange(1, 21), np.arange(1, 21)])#
    pc = LinePlotChecker(_globals["ax"])
    pc.assert_x_data_allclose(x)
    pc.assert_y_data_allclose(t) 
    print('Plot correct!!')
...
def question4(n1, n2, n3):
    ...
    try:
        ...
        print("alue for n1 is correct!!")
        score += 1
    try:
        assert(n2 == 'No pythagorean triplet satisfies that total')
    except:
        print("Value for n2 is incorrect!")
    else:
        print("Value for n2 is correct!!")
...
import time        
def question4_timed(x):
    start      = time.time()
    time_limit = 15
    assert((200, 375, 425, 31875000) == x)
